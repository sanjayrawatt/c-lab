...
int partition(int arr[], int low, int high) {
    ...
    for (int j = low; j < high; j++) {
        if (arr[j] > pivot) { // Change to arr[j] > pivot for Kth largest
            i++;
...
int quickSelect(int arr[], int low, int high, int k) {
    if (low <= high) {
        ...
        // If pivot index is the Kth largest element, return it
        if (pi == k - 1)
            return arr[pi];
        // If Kth largest element is less than pivot, recurse on the left subarray
        else if (k - 1 < pi)
...
int main() {
    ...
    if (k <= 0 || k > n) {
        ...
        // Find the (n - K + 1)th smallest element, which is the Kth largest element
